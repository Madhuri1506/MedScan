...
class CovidPnemonia:
    def __init__(self, frame):
        self.frame = frame
        self.model_covid_pnemonia = tf.keras.models.load_model(r'Covid_Pnemonia/Covid1.h5')
        ...
        self.width = 110
        self.height = 110
        self.img1_dir = ImageTk.PhotoImage(Image.open(r"Covid_Pnemonia/COVID19(3).jpg").resize((110, 110)), Image.ANTIALIAS)
        self.img1 = Label(self.frame, image=self.img1_dir, width=self.width, height=self.height)
        self.img2_dir = ImageTk.PhotoImage(Image.open(r"Covid_Pnemonia/NORMAL(0).jpg").resize((110, 110)), Image.ANTIALIAS)
        self.img2 = Label(self.frame, image=self.img2_dir, width=self.width, height=self.height)
        self.img3_dir = ImageTk.PhotoImage(Image.open(r"Covid_Pnemonia/PNEUMONIA(0).jpg").resize((110, 110)), Image.ANTIALIAS)
        ...
        self.img1_tag = Label(self.frame, text="Covid 19", font=tkinter.font.Font(family="Helvetica", size=9, weight="normal"))
        self.img2_tag = Label(self.frame, text="Normal", font=tkinter.font.Font(family="Helvetica", size=9, weight="normal"))
        self.img3_tag = Label(self.frame, text="Pneumonia", font=tkinter.font.Font(family="Helvetica", size=9, weight="normal"))
        ...
        self.img3_tag.place(x=400, y=190)

        # About Section
        self.label6i = Label(self.frame, text="About Covid 19 and Pneumonia",
                          font=tkinter.font.Font(family="Helvetica", size=15, weight="bold"))
        self.label6i.place(x=100, y=220)
        self.label6 = Label(self.frame, wraplength=500, justify=LEFT, text="""Covid 19
...
Symptoms include a cough with phlegm or pus, fever, chills and difficulty breathing.
        ...
        self.label7 = Label(self.frame, wraplength=500, justify=LEFT, text="""Covid 19
...
""")
        self.label7.place(x=6, y=418)
        self.label7i.place(x=185, y=392)

        # treatment Section
        self.label9i = Label(self.frame, text="Treatment", font=tkinter.font.Font(family="Helvetica", size=15, weight="bold"))
        self.label9 = Label(self.frame, wraplength=500, justify=LEFT, text="""Covid 19
...
Antibiotics can treat many forms of pneumonia. Some forms of pneumonia can be prevented by vaccines.""")
        self.label9.place(x=5, y=580)
        self.label9i.place(x=185, y=555)

        # right Section
        self.label10 = Label(self.frame, text="COVID 19/PNEUMONIA PREDICTION",
                          font=tkinter.font.Font(family="Helvetica", size=15, weight="bold"))
        self.label10.place(x=585, y=10)
        ...
        self.label16 = Label(self.frame, text="Covid:",
        ...
        self.label18 = Label(self.frame, text="Pnemonia:",
                             font=tkinter.font.Font(family="Helvetica", size=14, weight="bold"))
        ...
        self.img4 = ImageTk.PhotoImage(Image.open(r"Covid_Pnemonia/normal1.png").resize((20, 1)), Image.ANTIALIAS)
        self.border1 = Label(self.frame, image=self.img4, bg="black", bd=1, width=510, height=1)
        self.border1.place(x=0, y=210)
        self.img5 = ImageTk.PhotoImage(Image.open(r"Covid_Pnemonia/normal1.png").resize((20, 1)), Image.ANTIALIAS)
    ...
    def predict(self,img):
        ...
        prediction = self.model_covid_pnemonia.predict(new_array2)
        prediction = prediction[0]
        Covid = prediction[0]
        Normal = prediction[1]
        Pnemonia = prediction[2]
        if Covid > Normal and Covid > Pnemonia:
            predictionlabel = "Covid 19"
            predictionAccuracy = Covid * 100
        elif Pnemonia > Normal and Pnemonia > Covid:
            predictionlabel = "Pnemonia"
            predictionAccuracy = Pnemonia * 100
        else:
